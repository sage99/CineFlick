  ...
  <v-container v-if="TVShowDetails && Object.keys(TVShowDetails).length > 0" class="white--text">
    <v-layout id="tvDetails" row wrap class="mb-5">
      ...
      <v-flex class='ml-5' xs7>
        ...
        <v-tooltip bottom>
          <v-btn @click="eventBus.$emit('addToPlaylist', { data: TVShowDetails, type: 'tv' })" slot="activator" dark outline fab>
            <v-icon>add</v-icon>
          </v-btn>
      ...
      <v-flex mt-3 ml-3 xs4>
        <v-card class="br20">
          ...
          <v-card-text>
            ...
            <v-chip @click="searchGenre(genre)" color="teal lighten-2" class="mt-3 " v-for="(genre, index2) in TVShowDetails.genres" :key="index2 + 'c'">
...
export default {
  ...
  methods: {
    ...
    async searchGenre (genre) {
      // this.isLoading[index] = true
      await this.$store.dispatch('ACTION_SEARCH_GENRE', { type: 'TV', id: genre.id })
      // this.isLoading[index] = false
      // this.$emit('closeDialog')
